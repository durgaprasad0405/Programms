...
public class PostPetImage extends Global {
	...
	public PostPetImage() throws FileNotFoundException, IOException, ParseException {
		...
		for (rowNum = 1; rowNum <= tdSheet.getLastRowNum(); rowNum++) {
			...
			if ("Yes".equalsIgnoreCase(executionFlag)) {
				try {
					...
					resourcePath = resourcePath + "/" + inputPayloadElementsMap.get("payload_petId")+"/uploadImage";
					String filePath = inputPayloadElementsMap.get("payload_image");
					System.out.println(filePath);
					System.out.println(baseURI+resourcePath);
							
					response = restServices.postMethodFile(baseURI, resourcePath, contentType, filePath, password,
							inputPayload);
