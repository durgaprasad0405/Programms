...
public class PutPet extends Global {
	...
	public PutPet() throws FileNotFoundException, IOException, ParseException {
...
//					resourcePath = resourcePath + "?key=" + inputPayloadElementsMap.get("payload_key");
					...
					status = jsonResultMap.get("status").toString();
					...
					if ("Yes".equalsIgnoreCase(inputPayloadElementsMap.get("IsGetRequired?"))) 
						...
						cn = excelOps.getColNumByColName(excelFileName, sheetName, "payload_status");
						rn = excelOps.getRowNumByCelVal(excelFileName, sheetName, testCaseNum);
						excelOps.setCellValue(excelFileName, sheetName, rn, cn, status);
