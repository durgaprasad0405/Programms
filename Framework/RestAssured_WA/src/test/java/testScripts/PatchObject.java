...
public class PatchObject extends Global {
	...
	public PatchObject() throws FileNotFoundException, IOException, ParseException {
		...
		for (rowNum = 1; rowNum <= tdSheet.getLastRowNum(); rowNum++) {
			...
			if ("Yes".equalsIgnoreCase(executionFlag)) {
				try {
					...
					response = restServices.patchMethod(baseURI, resourcePath, contentType, userName, password,
							inputPayload);
